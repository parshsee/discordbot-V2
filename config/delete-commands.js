...
(async () => {
	try {
		...
		await rest.delete(
			Routes.applicationGuildCommand(process.env.DISCORD_CLIENTID, process.env.DISCORD_GUILDID, 'commandId'),
		);

		// // Delete a command from all guilds
		// await rest.delete(
		// 	Routes.applicationCommand(process.env.DISCORD_CLIENTID, 'commandId'),
		// );
