...
const exportedMethods = {
	...
	async execute(guildMember) {
		...
		if (!guild.available) {
			console.log('Error: Guild not available for member logging (remove)');
			return;
		}
		...
		if (!memberLogChannel) {
			...
			console.log(`Error: ${guild.name} Guild does not have member-log channel for logging (remove)`);
			...
			if (!sysChannel) {
				...
				if (!backupChannel) {
					console.log(`Error: ${guild.name} Guild has no text channels that bot can send messages to. member-logging (remove) not performed`);
					return;
				}
				// Send error message to backup channel that required channels are not available
				return backupChannel.send({ content: `Error: ${guild.name} does not have a member-log channel for logging (remove).\nA moderator can run the slash command '/create-required-channels' to create the necessary channels` });
			}

			// Send error message to backup channel that required channels are not available
			sysChannel.send({ content: `Error: ${guild.name} does not have a member-log channel for logging (remove).\nA moderator can run the slash command '/create-required-channels' to create the necessary channels` });
		...
		const embed = new EmbedBuilder()
			.setFooter({
				text: `${guildMember.user.tag} has left.`,
				iconURL: guildMember.user.displayAvatarURL(),
			});


		console.log(`${guildMember.user.tag} has left ${guild.name}`);
